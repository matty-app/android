...
class EmailNotExistsValidatorTest {

    @Test
    fun `email should not be valid if doesn't exist`() = runTest {
        val email = "john@gmail.com"
        val expectedError = StringResourceError(R.string.email_not_exists)
        val authService = mockk<AuthService>().apply {
            coEvery { checkEmailExists(email) } returns false
        }
        val validator = EmailNotExistsValidator(authService)
    ...
    @Test
    fun `email should be valid if exist`() = runTest {
        val email = "john@gmail.com"
        val authService = mockk<AuthService>().apply {
